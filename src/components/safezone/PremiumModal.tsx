      ...
      <div className="relative w-full max-w-md bg-[#0f1622] border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 pb-8 space-y-4">
          {!paid ? (
            ...
            >
              {loading ? 'PayPal wird geöffnet...' : 'Mit PayPal bezahlen'}
            </button>
          ) : (
            <>
              <div className="text-center text-green-400 font-medium">
                ✅ PayPal geöffnet<br />
                Bitte bezahle dort und bestätige danach hier:
              </div>
              <button
                onClick={handleConfirmPayment}
                className="w-full py-4 rounded-2xl font-bold text-lg bg-green-600 hover:bg-green-500 text-white"
              >
                Ich habe bezahlt – Premium freischalten
              </button>
            </>
          )}

          <p className="text-center text-slate-500 text-xs">
            Nach erfolgreicher Zahlung wird Premium sofort freigeschaltet
          </p>
        </div>
      </div>
    </div>
